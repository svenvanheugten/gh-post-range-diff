...
-- the head still sits on -- i.e. every ordinary shape, and it stays shallow.
--
...
-- --shallow-exclude, so after the fast path no recorded base is yet an ancestor of
-- that head. That is cheap to detect (the anyM checks below) and rare, so we just
-- --unshallow the affected heads to pull their full history, but we don't immediately
-- fetch the blobs.
...
unshallowRange base oldHead newHead baseOids = do
    ...
    when shallow $ do
        ...
        unless (null baseOids) $ do
            oldOk <- anyM (`isAncestor` oldHead) baseOids
            newOk <- anyM (`isAncestor` newHead) baseOids
            unless (oldOk && newOk) $
                callProcess "git" ["fetch", "--quiet", "--unshallow", "--filter=blob:none", "origin", oldHead, newHead]
