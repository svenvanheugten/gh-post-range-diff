...
import GhPostRangeDiff.RangeDiff (Change (..), Commit (..), CommitSha, commitSha, interdiff, rangeDiff)
...
bigInterdiff :: String
bigInterdiff = unlines ["@@ big.txt (new)", " +l6", " +l7", " +l8", "-+```OLD", "++```NEW"]
...
spec = describe "rangeDiff" $ do
  ...
  it "reads a change, a sha from the surviving side, and a de-indented interdiff per commit" $
    commits
      `shouldBe` [ Commit (Updated (interdiff bigInterdiff)) newBig "big feature",
