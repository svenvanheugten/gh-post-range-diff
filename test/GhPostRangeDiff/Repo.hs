...
-- | Run jj, handing back what it printed on stdout. jj narrates what it
-- rewrote on stderr, which is noise while the build goes to plan and the whole
-- story when it doesn't, so it is kept back until a command fails.
jj :: [String] -> IO String
jj args = do
  (code, out, err) <- readProcessWithExitCode "jj" args ""
  case code of
...
initRepo :: IO ()
initRepo = do
  -- jj makes a commit as it initialises the repo, so who it commits as has to
  -- be settled on that very command line; the config set below only reaches the
  -- commits after it.
  let asAuthor = concat [["--config", k ++ "=" ++ v] | (k, v) <- identity]
  _ <- jj (asAuthor ++ ["git", "init", "--colocate", "."])
  mapM_ (\(k, v) -> jj ["config", "set", "--repo", k, v]) settings
  ...
  pure ()
  where
    identity = [("user.name", auName author), ("user.email", auEmail author)]

    -- Everything the build leans on, settled here rather than left to the jj
    -- config of whoever is running the tests, since a repo's own settings win
    -- over a user's.
    settings =
      identity
        ++ [ -- Every file a commit writes is a file it means to commit.
             ("snapshot.auto-track", "all()"),
             -- Rewriting history is what this repo is for, so nothing in it is
